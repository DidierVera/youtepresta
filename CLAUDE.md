...
- Fase en curso: **Fase 3** (pendiente de definir alcance — amigos/préstamos).
- Última fase completada: **Fase 2 — Bolsillos (funding sources)**. `FundingSourceRepository` (singleton) con `getFundingSources`, `createFundingSource` (inserta con `current_balance = 0` y registra el saldo inicial como movimiento "Saldo inicial" si es > 0), `getSourceMovements`, `addManualMovement` (inserta el movimiento y luego actualiza `current_balance`, sin transacción atómica). `FundingSourcesScreen`/`ViewModel` (lista + crear con bottom sheet) y `FundingSourceDetailScreen`/`ViewModel` (historial + agregar/retirar dinero con bottom sheet) implementados en `ui/sources`. Montos formateados con `NumberFormat` es-CO. Rutas `sources` y `sources/{sourceId}` agregadas al `NavHost`; `HomeScreen` tiene botón temporal "Ver bolsillos". No se tocó RLS: `user_id` nunca se envía desde el cliente, el default `auth.uid()` de la tabla funciona solo porque Postgrest reutiliza el access token de la sesión de `Auth` automáticamente. Flujo completo (crear bolsillo con saldo inicial → detalle → agregar y retirar dinero → historial → balance) probado en dispositivo físico contra Supabase real.
...
- Queda en la base de datos de Supabase un bolsillo de prueba ("Presupuesto para prestamos", con movimientos "Saldo inicial", "Dinero 50000" y "Retiro de prueba") creado durante las pruebas de esta fase — el usuario decidió no borrarlo por ahora.
